...
function Contact() {
  ...
  const handleSubmit = async (e) => {
    ...
    return (
        <section className="w-full flex flex-col lg:flex-row items-center justify-evenly p-4 gap-24" id="contact">
          {isVisible && (
          ...
          <div className="relative flex flex-col bg-theme-1-3 w-full h-auto rounded-tl-[55%] rounded-tr-[68%] rounded-bl-[56%] rounded-br-[43%] rounded-custom">
            ...
            <a href="tel:917306310132" className="font-bold w-auto text-md xs:text-2xl custom-pointer">+91 7306310132</a>
            <span className="font-bold text-xs xs:text-xl text-theme-1-2"><br />Email</span>
            <a href="mailto:krishnasankarofficial@gmail.com?subject=Subject%20of%20the%20Email&body=Hello%20there!" className="font-bold text-sm xs:text-2xl z-10 custom-pointer">krishnasankarofficial@gmail.com</a>
            <br />
          </div>
          <form ref={form} onSubmit={handleSubmit} className="w-full flex flex-col gap-4">
            ...
            <div className="flex flex-col md:flex-row text-xl w-full items-start gap-4 mt-10">
              <input className="bg-transparent border-2 border-theme-1-3 p-4 rounded-2xl w-full custom-text" 
                type="text" 
                name="user_name" 
                id="user_name" 
                value={username}
            ...
            <div className="flex xs:flex-col md:flex-row text-xl w-full items-start gap-4">
              <textarea className="bg-transparent border-2 border-theme-1-3 p-4 rounded-2xl w-full h-52 items-start justify-start custom-text" 
                type="text" 
                name="message" 
                id="message" 
                value={message}
            ...
            <button className="flex w-full h-20 bg-theme-1-4 border-2 border-theme-1-1 hover:bg-theme-1-3 items-center justify-center text-2xl font-bold rounded-2xl custom-pointer">Send</button>
          </form>
        </section>
    )
}
